...
为恢复宽页面板式，将全局 `--max-w` 从 960 px 调整为 1200 px。摘要段落限制为 920 px，章节导语、任务正文、图注和诊断说明限制为 960 px 并居中，避免宽屏下正文行过长。任务演示区仍使用 1280 px。1440 px 桌面截图确认主体宽度展开，移动端页面保持无整体横向溢出。
